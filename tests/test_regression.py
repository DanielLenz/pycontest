...
def test_regression():
    ...
    loc_path = Path(os.path.join(os.path.dirname(__file__),"../data/loc_ref.npy"))
    vel_path = Path(os.path.join(os.path.dirname(__file__),"../data/vel_ref.npy"))

    if not (loc_path.is_file() and vel_path.is_file()):
        np.save(loc_path, loc)
        np.save(vel_path, vel)
        return
    else:
        loc_ref = np.load(loc_path)
        vel_ref = np.load(vel_path)
        ...
        np.testing.assert_allclose(vel, vel_ref, atol=0, rtol=1e-17, err_msg="for velocities")
